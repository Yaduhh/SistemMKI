...
@if ($message)
    <div class="bg-{{ $type === 'success' ? 'green-100' : 'red-100' }} border-l-4 border-{{ $type === 'success' ? 'green-500' : 'red-500' }} text-{{ $type === 'success' ? 'green-800' : 'red-800' }} px-4 py-3 rounded-md flex items-center justify-between shadow-sm" role="alert" x-data="{ show: true }" x-show="show" x-transition>
