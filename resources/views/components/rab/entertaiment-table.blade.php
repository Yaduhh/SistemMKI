<div>
    ...
    <button type="button" class="dark:bg-teal-900 bg-teal-600 text-white w-full py-2 uppercase" id="add-entertaiment-mr-group">Tambah MR</button>
    <div class="flex justify-end bg-teal-600/10 py-2 px-4">
        <div class="font-semibold mr-2">Grand Total:</div>
        <div class="font-bold text-teal-700 dark:text-teal-400" id="grand-total-entertaiment">0</div>
    </div>
        ...
        <div class="w-full relative mr-group">
            <div class="flex flex-col md:flex-row md:items-center gap-4 mb-4">
                <div class="w-full">
                    ...
                    <input type="text" name="json_pengeluaran_entertaiment[__MRIDX__][mr]" placeholder="MR 001" class="border w-full rounded-xl px-4 py-2 dark:bg-zinc-800 dark:border-zinc-700 dark:text-white" required />
                </div>
                <div class="w-full">
                    <label class="block text-sm font-medium mb-1">Tanggal</label>
                    <input type="date" name="json_pengeluaran_entertaiment[__MRIDX__][tanggal]" class="border rounded-xl w-full px-4 py-2 dark:bg-zinc-800 dark:border-zinc-700 dark:text-white" required />
                </div>
                <div>
            ...
            <button type="button" class="dark:bg-teal-900 bg-teal-600 border border-teal-600 text-teal-100 hover:bg-teal-600 hover:text-white transition-all duration-300 rounded-xl px-4 py-2 add-entertaiment-material-row mt-6">Tambah Entertaiment</button>
            <div class="mt-4 flex justify-end border-t border-b border-teal-600/30 bg-teal-600/10 px-4 py-2">
    ...
    <template id="entertaiment-material-row-template">
        <div class="grid grid-cols-1 md:grid-cols-6 gap-4 items-end p-6 rounded-xl bg-white dark:bg-zinc-700/30 relative entertaiment-material-row pt-12">
            ...
            <div>
                ...
                <input type="number" min="0" data-material-field="harga_satuan" name="json_pengeluaran_entertaiment[__MRIDX__][materials][__MATIDX__][harga_satuan]" placeholder="Harga Satuan" class="w-full border rounded-lg px-2 py-1 dark:bg-zinc-800 dark:border-zinc-700 dark:text-white harga-input" />
            </div>
            <div>
                <label class="block text-xs font-medium mb-1">Sub Total</label>
                <input type="number" data-material-field="sub_total" name="json_pengeluaran_entertaiment[__MRIDX__][materials][__MATIDX__][sub_total]" placeholder="Sub Total" class="w-full border rounded-lg px-2 py-1 dark:bg-zinc-800 dark:border-zinc-700 dark:text-white sub-total-input" readonly />
            </div>
        ...
        (function () {
            ...
            function renderAllNames() {
                mrList.querySelectorAll('.mr-group').forEach((mrGroup, mrIdx) => {
                    ...
                    mrGroup.querySelectorAll('input[name^="json_pengeluaran_entertaiment["]').forEach(input => {
                        let name = input.getAttribute('name');
                        name = name.replace(/json_pengeluaran_entertaiment\[\d+\]/, `json_pengeluaran_entertaiment[${mrIdx}]`);
                        name = name.replace(/materials\]\[\d+\]/g, match => match.replace(/\d+/, '0'));
                        input.setAttribute('name', name);
                    });
                    // Update material row names
                    mrGroup.querySelectorAll('.entertaiment-material-row').forEach((row, matIdx, arr) => {
            ...
            mrList.addEventListener('click', function (e) {
                ...
                if (e.target.classList.contains('remove-material')) {
                    const mrGroup = e.target.closest('.mr-group');
                    const matList = mrGroup.querySelector('.entertaiment-material-list');
                    const rows = matList.querySelectorAll('.entertaiment-material-row');
                    if (rows.length > 1) {
                        e.target.closest('.entertaiment-material-row').remove();
                        renderAllNames();
                    }
                }
            });
