<div>
    <div id="lainnya-mr-list" class="space-y-8">
        <!-- MR group akan di-generate oleh JS -->
    </div>
    <button type="button" class="dark:bg-pink-900 bg-pink-600 text-white w-full py-2 uppercase" id="add-lainnya-mr-group">Tambah MR</button>
    <div class="flex justify-end bg-pink-600/10 py-2 px-4">
        <div class="font-semibold mr-2">Grand Total:</div>
        <div class="font-bold text-pink-700 dark:text-pink-400" id="grand-total-lainnya">0</div>
    </div>
    <template id="lainnya-mr-group-template">
        <div class="w-full relative mr-group">
            <div class="flex flex-col md:flex-row md:items-center gap-4 mb-4">
                <div class="w-full">
                    <label class="block text-sm font-medium mb-1">MR</label>
                    <input type="text" data-mr-field="mr" name="json_pengeluaran_lainnya[__MRIDX__][mr]" placeholder="MR 001" class="border w-full rounded-xl px-4 py-2 dark:bg-zinc-800 dark:border-zinc-700 dark:text-white" />
                </div>
                <div class="w-full">
                    <label class="block text-sm font-medium mb-1">Tanggal</label>
                    <input type="date" data-mr-field="tanggal" name="json_pengeluaran_lainnya[__MRIDX__][tanggal]" class="border rounded-xl w-full px-4 py-2 dark:bg-zinc-800 dark:border-zinc-700 dark:text-white" />
                </div>
                <div>
            ...
            <div class="space-y-0 lainnya-material-list">
                <!-- Baris lainnya akan di-generate oleh JS -->
            </div>
            <button type="button" class="dark:bg-pink-900 bg-pink-600 border border-pink-600 text-pink-100 hover:bg-pink-600 hover:text-white transition-all duration-300 rounded-xl px-4 py-2 add-lainnya-material-row mt-6">Tambah Lainnya</button>
            <div class="mt-4 flex justify-end border-t border-b border-pink-600/30 bg-pink-600/10 px-4 py-2">
                <div class="font-semibold mr-2">Total MR :</div>
                <div class="font-bold text-pink-600 dark:text-pink-400 total-mr">0</div>
            </div>
        </div>
    </template>
    <template id="lainnya-material-row-template">
        <div class="grid grid-cols-1 md:grid-cols-6 gap-4 items-end p-6 rounded-xl bg-white dark:bg-zinc-700/30 relative lainnya-material-row pt-12">
            <div>
                <label class="block text-xs font-medium mb-1">Supplier</label>
                <input type="text" data-material-field="supplier" name="json_pengeluaran_lainnya[__MRIDX__][materials][__MATIDX__][supplier]" placeholder="Supplier" class="w-full border rounded-lg px-2 py-1 dark:bg-zinc-800 dark:border-zinc-700 dark:text-white" />
            </div>
            <div>
                <label class="block text-xs font-medium mb-1">Item</label>
                <input type="text" data-material-field="item" name="json_pengeluaran_lainnya[__MRIDX__][materials][__MATIDX__][item]" placeholder="Item" class="w-full border rounded-lg px-2 py-1 dark:bg-zinc-800 dark:border-zinc-700 dark:text-white" />
            </div>
            <div>
                <label class="block text-xs font-medium mb-1">Qty</label>
                <input type="number" min="0" data-material-field="qty" name="json_pengeluaran_lainnya[__MRIDX__][materials][__MATIDX__][qty]" placeholder="Qty" class="w-full border rounded-lg px-2 py-1 dark:bg-zinc-800 dark:border-zinc-700 dark:text-white qty-input" />
            </div>
            <div>
                <label class="block text-xs font-medium mb-1">Satuan</label>
                <input type="text" data-material-field="satuan" name="json_pengeluaran_lainnya[__MRIDX__][materials][__MATIDX__][satuan]" placeholder="Satuan" class="w-full border rounded-lg px-2 py-1 dark:bg-zinc-800 dark:border-zinc-700 dark:text-white" />
            </div>
            <div>
    ...
    <script>
        (function () {
            const mrList = document.getElementById('lainnya-mr-list');
            const addMrBtn = document.getElementById('add-lainnya-mr-group');
            const mrGroupTemplate = document.getElementById('lainnya-mr-group-template');
            ...
            function renderAllNames() {
                mrList.querySelectorAll('.mr-group').forEach((mrGroup, mrIdx) => {
                    ...
                    mrGroup.querySelectorAll('input[name^="json_pengeluaran_lainnya["]').forEach(input => {
                        let name = input.getAttribute('name');
                        name = name.replace(/json_pengeluaran_lainnya\[\d+\]/, `json_pengeluaran_lainnya[${mrIdx}]`);
                        name = name.replace(/materials\]\[\d+\]/g, match => match.replace(/\d+/, '0'));
                        input.setAttribute('name', name);
                    });
                    // Update material row names
                    mrGroup.querySelectorAll('.lainnya-material-row').forEach((row, matIdx, arr) => {
            ...
            function addMaterialRow(mrGroup, data) {
                ...
                const matList = mrGroup.querySelector('.lainnya-material-list');
                const matIdx = matList.querySelectorAll('.lainnya-material-row').length;
            ...
            if (window.oldLainnya && Array.isArray(window.oldLainnya) && window.oldLainnya.length) {
                window.oldLainnya.forEach(mr => addMrGroup(mr));
            ...
            mrList.addEventListener('click', function (e) {
                ...
                if (e.target.classList.contains('add-lainnya-material-row')) {
                ...
                if (e.target.classList.contains('remove-material')) {
                    const mrGroup = e.target.closest('.mr-group');
                    const matList = mrGroup.querySelector('.lainnya-material-list');
                    const rows = matList.querySelectorAll('.lainnya-material-row');
                    if (rows.length > 1) {
                        e.target.closest('.lainnya-material-row').remove();
                        renderAllNames();
                    }
                }
            });
