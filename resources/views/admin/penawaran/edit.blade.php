<x-layouts.app>
    <x-slot name="header">
        <div class="flex justify-between items-center mb-4">
            <h1 class="text-xl font-bold text-gray-900 dark:text-white">Edit Penawaran</h1>
        ...
        <form action="{{ route('admin.penawaran.update', $penawaran->id) }}" method="POST" class="space-y-8" x-data="penawaranForm()" x-init="init()">
            ...
            <div class="w-full">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <!-- Nomor Penawaran (Hidden) -->
                    <input type="hidden" name="nomor_penawaran" value="{{ $penawaran->nomor_penawaran }}">
                    <!-- Client -->
                    ...
                    <flux:input name="judul_penawaran" label="Judul Penawaran" placeholder="masukkan judul penawaran" type="text" value="{{ $penawaran->judul_penawaran }}" required class="dark:bg-zinc-800 dark:border-zinc-700 dark:text-white" />
                        ...
                        <template x-if="mainSection.productSections.length > 0">
                            <template x-for="(section, sIdx) in mainSection.productSections" :key="section.kategori">
                                <div class="w-full mb-6" :class="{
                                    ...
                                    'border-cyan-400': section.kategori === 'rotan_sintetis',
                                }">
                                    <div class="flex justify-between items-center mb-4">
                    ...
                    <div class="p-6 border-b border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800/50">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <input type="number" name="diskon_satu" x-model="diskon_satu" @input="handleDiskon1Input($event)" step="0.01" class="w-full py-2 px-3 rounded-lg border-2 border-gray-300 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white focus:ring-2 focus:ring-blue-400 focus:border-blue-400" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Diskon 2 (%)</label>
                                <input type="number" name="diskon_dua" x-model="diskon_dua" @input="handleDiskon2Input($event)" step="0.01" class="w-full py-2 px-3 rounded-lg border-2 border-gray-300 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white focus:ring-2 focus:ring-blue-400 focus:border-blue-400" />
                            </div>
                        </div>
...
function penawaranForm() {
    return {
        ...
        loadClients(userId) {
            ...
            fetch(`/admin/penawaran/clients/${userId}`)
                ...
                .catch(error => {
                    ...
                    const clientSelect = document.querySelector('select[name="id_client"]');
                    clientSelect.innerHTML = '<option value="" selected>Error loading clients</option>';
                });
        },
        ...
        getAvailableHollows(condition, row) {
            // Untuk edit, tampilkan semua hollow termasuk yang sudah dipilih
