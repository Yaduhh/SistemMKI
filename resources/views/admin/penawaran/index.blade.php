<x-layouts.app>
    ...
    <div class="pb-6 space-y-6">
            ...
            <div class="bg-green-50 dark:bg-green-900/20 rounded-xl p-6 border border-green-200 dark:border-green-800">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $penawarans->where('status', 1)->count() }}</p>
                    </div>
            ...
            <div class="bg-red-50 dark:bg-red-900/20 rounded-xl p-6 border border-red-200 dark:border-red-800">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $penawarans->where('status', 2)->count() }}</p>
                    </div>
            ...
            <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl p-6 border border-yellow-200 dark:border-yellow-800">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $penawarans->where('status', 0)->count() }}</p>
                    </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-zinc-900/30 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-xl transition-all duration-300">
                <div class="p-6">
                    <!-- Header Row -->
                    <div class="flex items-center justify-between mb-6">
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                <h3 class="text-xl font-bold text-gray-900 dark:text-white">{{ $penawaran->judul_penawaran }}</h3>
                                <p class="text-sm text-gray-600 dark:text-gray-400 font-mono">{{ $penawaran->nomor_penawaran }}</p>
                            </div>
                        </div>
                        ...
                        <div class="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $penawaran->client->nama ?? 'Client tidak ditemukan' }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-green-50 dark:bg-green-900/20 rounded-xl p-4">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $penawaran->user->name ?? 'Sales tidak ditemukan' }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-4">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $penawaran->tanggal_penawaran ? $penawaran->tanggal_penawaran->format('d M Y') : 'Belum ditentukan' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 mb-6">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div class="text-center">
                                ...
                                <p class="text-lg font-bold text-gray-900 dark:text-white">Rp {{ number_format($penawaran->total, 0, ',', '.') }}</p>
                            </div>
                            <div class="text-center">
                                <p class="text-sm text-gray-600 dark:text-gray-400">Diskon</p>
                                <p class="text-lg font-bold text-red-600 dark:text-red-400">- Rp {{ number_format(($penawaran->total_diskon + ($penawaran->total_diskon_1 ?? 0) + ($penawaran->total_diskon_2 ?? 0)), 0, ',', '.') }}</p>
                            </div>
                            <div class="text-center">
                                <p class="text-sm text-gray-600 dark:text-gray-400">PPN</p>
                                <p class="text-lg font-bold text-blue-600 dark:text-blue-400">+ Rp {{ number_format((($penawaran->total - ($penawaran->total_diskon + ($penawaran->total_diskon_1 ?? 0) + ($penawaran->total_diskon_2 ?? 0) ))*$penawaran->ppn/100), 0, ',', '.') }}</p>
                            </div>
                            <div class="text-center">
                                <p class="text-sm text-gray-600 dark:text-gray-400">Grand Total</p>
                                <p class="text-xl font-bold text-green-600 dark:text-green-400">Rp {{ number_format($penawaran->grand_total, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        <form action="{{ route('admin.penawaran.destroy', $penawaran) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus penawaran ini?')">
                            @csrf @method('DELETE')
                            <x-button type="submit" class="bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:hover:bg-red-900/30 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 px-4 py-2 rounded-lg transition-all duration-300 hover:shadow-md transform hover:-translate-y-0.5">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="col-span-full">
                <div class="bg-white dark:bg-zinc-900/30 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-12 text-center">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 mb-8">Mulai dengan membuat penawaran pertama Anda untuk mengelola bisnis dengan lebih efisien</p>
                    <x-button href="{{ route('admin.penawaran.create') }}" class="bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-300 px-8 py-3">
