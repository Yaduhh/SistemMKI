<x-layouts.app>
    ...
    <div class="py-4 mx-auto w-full">
        <form action="{{ route('admin.penawaran-pintu.update', $penawaran->id) }}" method="POST" class="space-y-8"
            ...
            <div class="w-full">
                ...
                <div class="grid grid-cols-1 gap-4">
                    @if($penawaran->syarat_kondisi && is_array($penawaran->syarat_kondisi))
                        @foreach ($penawaran->syarat_kondisi as $syarat)
                            <div class="flex items-center">
                                <input type="checkbox" name="syarat_kondisi[]" value="{{ $syarat }}"
                                    id="syarat_{{ $loop->index }}"
                                    checked
                                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                <label for="syarat_{{ $loop->index }}"
    ...
    <script>
        function penawaranForm() {
            return {
                ...
                init() {
                    try {
                        // Load data dari penawaran yang sudah ada
                        this.loadDataFromExisting();
                    } catch (error) {
                        console.error('❌ Error saat loading data:', error);
                        // Fallback: tambah section kosong jika error
                        this.addPintuSection();
                    }
                    
                    // Tambahkan event listener untuk input PPN
                    this.$nextTick(() => {
                        const ppnInput = document.querySelector('input[name="ppn"]');
                        if (ppnInput) {
                            ppnInput.addEventListener('input', (e) => {
                                this.calculateTotal();
                            });
                        }
                    });
                ...
                // Load data dari penawaran yang sudah ada
                loadDataFromExisting() {
                    this.isLoadingData = true;
                    const existingData = @json($penawaran->json_penawaran_pintu ?? '{}');
                    
                    if (existingData && Object.keys(existingData).length > 0) {
                        // Convert object keys to array and sort by section number
                        const sections = Object.keys(existingData)
                        ...
                        // Buat semua section terlebih dahulu
                        sections.forEach((sectionKey, sectionIndex) => {
                            const section = existingData[sectionKey];
                            // Tambah section baru
                            this.addPintuSection();
                        });
                        
                        // Tunggu semua section selesai dibuat
                        this.$nextTick(() => {
                            // Set nilai section dan produk
                            sections.forEach((sectionKey, sectionIndex) => {
                                const section = existingData[sectionKey];
                                ...
                                // Set nilai produk dalam section
                                if (section.products && Array.isArray(section.products)) {
                                    // Hapus produk pertama yang otomatis ditambahkan
                                    const container = document.getElementById(`pintu-products-${sectionIndex}`);
                                    if (container && container.children.length > 0) {
                                        container.removeChild(container.children[0]);
                                        this.pintuSections[sectionIndex].products.splice(0, 1);
                                    }
                                    
                                    // Tambah semua produk sesuai data yang ada
                                    section.products.forEach((produk, produkIndex) => {
                                        this.addPintuProduk(sectionIndex);
                                    });
                                    
                                    // Tunggu semua produk selesai ditambahkan
                                    this.$nextTick(() => {
                                        // Set nilai produk setelah DOM selesai
                                        section.products.forEach((produk, produkIndex) => {
                            ...
                            // Set nilai PPN dari database
                            this.ppn = {{ $penawaran->ppn ?? 11 }};
                            
                            // Hitung ulang setelah semua data di-load
                            this.$nextTick(() => {
                ...
                setProductValue(sectionIndex, productIndex, produk) {
                    ...
                    if (container) {
                        ...
                        if (productDiv) {
                            ...
                            if (namaProdukInput) {
                                namaProdukInput.value = produk.nama_produk || '';
                            }
                            ...
                            // Set jumlah individual
                            const jumlahIndividualInput = productDiv.querySelector('input[name*="[jumlah_individual]"]');
                            const jumlahIndividualContainer = productDiv.querySelector(`#jumlah-individual-container-${sectionIndex}-${productIndex}`);
                            if (jumlahIndividualInput && jumlahIndividualContainer) {
                                jumlahIndividualInput.value = produk.jumlah_individual || 1;
                                
                                // Cek apakah produk ini aksesoris untuk menentukan visibility
                                if (produk.item) {
                                    const selectedProduct = this.products.pintu.find(p => p.code === produk.item);
                                    if (selectedProduct && selectedProduct.status_aksesoris == 1) {
                                        jumlahIndividualContainer.style.display = 'block';
                                    } else {
                                        jumlahIndividualContainer.style.display = 'none';
                                    }
                                }
                            }
                            ...
                            if (totalInput) {
                                totalInput.value = produk.total_harga || 0;
                                totalInput.dataset.rawValue = produk.total_harga || 0;
                            }
                        }
                    }
                },
                ...
                addPintuSection() {
                    ...
                    if (jumlahSectionInput) {
                        ...
                        jumlahSectionInput.addEventListener('change', function() {
                            self.calculateSubtotal();
                        });
                    }
                ...
                addPintuProduk(sectionIndex) {
                    const container = document.getElementById(`pintu-products-${sectionIndex}`);
                    if (!container) {
                        console.error(`Container tidak ditemukan untuk section ${sectionIndex}`);
                        return;
                    }
                    ...
                    productDiv.innerHTML = `
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mt-4">
                            <div id="jumlah-individual-container-${sectionIndex}-${productIndex}" style="display: none;">
                    ...
                    if (jumlahIndividualInput) {
                        jumlahIndividualInput.addEventListener('input', function() {
                            ...
                            // Panggil calculateSubtotal setelah menghitung total produk
                            self.calculateSubtotal();
                        });
                    }
                ...
                recalculateSectionProducts(sectionIndex) {
                    try {
                        ...
                        if (container) {
                            ...
                            // Log setiap produk yang akan di-recalculate
                            productDivs.forEach((productDiv, productIndex) => {
                                try {
                                    // Panggil calculatePintuProductTotal tanpa memanggil calculateSubtotal
                                    this.calculatePintuProductTotalWithoutSubtotal(sectionIndex, productIndex);
                                } catch (productError) {
                                    console.error(`❌ Error saat recalculate produk ${productIndex} di section ${sectionIndex}:`, productError);
                                }
                            });
                        } else {
                        ...
                        console.error(`❌ Error dalam recalculateSectionProducts section ${sectionIndex}:`, error);
                    }
                },

                recalculateAll() {
                    try {
                        // Recalculate semua produk tanpa memanggil calculateSubtotal
                        this.pintuSections.forEach((section, sectionIndex) => {
                            try {
                                this.recalculateSectionProducts(sectionIndex);
                            } catch (sectionError) {
                                console.error(`❌ Error saat recalculate section ${sectionIndex}:`, sectionError);
                            }
                        });
                        
                        // Hitung subtotal dan total sekali saja di akhir
                        this.calculateSubtotal();
                    } catch (error) {
                        console.error('❌ Error dalam recalculateAll:', error);
                    }
                },
                 ...
                 autofillPintuProduct(sectionIndex, productIndex, id) {
                    ...
                    if (produk) {
                        ...
                        if (productDiv) {
                            ...
                            // Update subtotal setelah mengisi data produk
                            this.calculateSubtotal();
                        }
                    }
                },
                ...
                calculatePintuProductTotal(sectionIndex, productIndex) {
                    try {
                        ...
                        const productDiv = container.children[productIndex];
                        if (!productDiv) {
                            console.error(`❌ Product div tidak ditemukan untuk index ${productIndex}`);
                            return;
                        }
                        ...
                        if (hargaInput && totalInput) {
                            ...
                            // Panggil calculateSubtotal untuk update total keseluruhan
                            this.calculateSubtotal();
                        } else {
                            console.error(`❌ Input harga atau total tidak ditemukan untuk produk ${productIndex}`);
                        }
                    } catch (error) {
                        console.error(`❌ Error dalam calculatePintuProductTotal ${sectionIndex}-${productIndex}:`, error);
                    }
                },

                // Fungsi untuk menghitung total produk tanpa memanggil calculateSubtotal
                calculatePintuProductTotalWithoutSubtotal(sectionIndex, productIndex) {
                    try {
                        const container = document.getElementById(`pintu-products-${sectionIndex}`);
                        if (!container) {
                            console.error(`❌ Container section ${sectionIndex} tidak ditemukan`);
                        ...
                        console.error(`❌ Error dalam calculatePintuProductTotalWithoutSubtotal ${sectionIndex}-${productIndex}:`, error);
                    }
                },
                ...
                calculateSubtotal() {
                    try {
                        ...
                        this.pintuSections.forEach((section, sectionIndex) => {
                            const container = document.getElementById(`pintu-products-${sectionIndex}`);
                            if (container) {
                                // Ambil jumlah section - cari dari parent section div
                                const sectionDiv = container.closest('.border-2.border-gray-300, .border-2.border-zinc-600');
                                const jumlahSectionInput = sectionDiv?.querySelector('input[name*="[jumlah]"]');
                                ...
                                productDivs.forEach((productDiv, productIndex) => {
                                    ...
                                    if (totalInput && totalInput.value) {
                                        ...
                                        const productTotal = parseFloat(totalInput.value) || 0;
                                        totalHargaSection += productTotal;
                                    }
                                });
                                
                                // Total section = total harga produk × jumlah section
                                const sectionTotal = totalHargaSection * jumlahSection;
                                this.subtotal += sectionTotal;
                            }
                        });
                        ...
                        // Panggil calculateTotal untuk menghitung grand total
                        this.calculateTotal();
                    } catch (error) {
                        console.error('❌ Error dalam calculateSubtotal:', error);
                    }
                },
                ...
                calculateTotal() {
                    try {
                        ...
                        console.error('❌ Error dalam calculateTotal:', error);
                    }
                },
                     ...
                     try {
                         ...
                         .then(data => {
                             if (data.success) {
                                 ...
                                 window.location.href = '{{ route("admin.penawaran-pintu.show", $penawaran->id) }}';
                             } else {
                                 // Tampilkan error
