        ...
        <div class="w-full mx-auto">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="space-y-3">
                                <div class="flex items-center">
                                    <span class="text-gray-900 dark:text-white">Nomor:
                                        {{ $pemasangan->nomor_pemasangan }}</span>
                                </div>
                                <div class="flex items-center">
                                    <span class="text-gray-900 dark:text-white">Tgl Pemasangan:
                                        {{ $pemasangan->tanggal_pemasangan ? $pemasangan->tanggal_pemasangan->format('d/m/Y') : '-' }}</span>
                                </div>
                                <div class="flex items-center">
                                    <span class="text-gray-900 dark:text-white">Judul:
                                        {{ $pemasangan->judul_pemasangan }}
                                    </span>
                                </div>
                                <div class="flex items-center">
                                    <span class="text-gray-900 dark:text-white">{{ $pemasangan->logo }}
                                    </span>
                                </div>
                            </div>
                            <div class="space-y-3">
                                <div class="flex items-center">
                                    <span class="text-gray-900 dark:text-white">Client:
                                        {{ $pemasangan->client->nama ?? '-' }}
                                    </span>
                                </div>
                                <div class="flex items-center">
                                    <span class="text-gray-900 dark:text-white">Sales:
                                        {{ $pemasangan->sales->name ?? '-' }}
                                    </span>
                                </div>
                                <div class="flex items-center">
                    ...
                    <div
                        ...
                        @if (is_array($pemasangan->json_pemasangan))
                            @foreach ($pemasangan->json_pemasangan as $section)
                                <div class="mb-6">
                                    <div class="font-semibold text-base text-gray-900 dark:text-white mb-2">
                                        {{ $section['sub_judul'] ?? '-' }}</div>
                                            ...
                                            <tbody
                                                ...
                                                @foreach ($section['items'] as $item)
                                                    <tr class="hover:bg-gray-50 dark:hover:bg-zinc-800/50">
                                                        ...
                                                        <td
                                                            class="px-6 py-4 whitespace-nowrap text-gray-900 dark:text-white">
                                                            {{ isset($item['harga_satuan']) ? 'Rp ' . number_format((float) $item['harga_satuan'], 0, ',', '.') : '-' }}
                                                        </td>
                                                        <td
                        ...
                        <div
                            ...
                            @else
                                <div class="text-gray-700 dark:text-gray-300 whitespace-pre-line">
                                    {{ $pemasangan->json_syarat_kondisi }}</div>
                            @endif
                        </div>
                    @endif
                </div>
            ...
            <div class="flex flex-wrap items-center gap-4 my-4">
                <a href="{{ route('admin.pemasangan.edit', $pemasangan->id) }}"
                    class="inline-flex items-center px-4 py-2 bg-indigo-600 dark:bg-indigo-600 border border-transparent rounded-lg font-semibold text-white hover:bg-indigo-700 dark:hover:bg-indigo-600 focus:bg-indigo-700 dark:focus:bg-indigo-600 active:bg-indigo-900 dark:active:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <form action="{{ route('admin.pemasangan.destroy', $pemasangan->id) }}" method="POST"
                    onsubmit="return confirm('Yakin ingin menghapus pemasangan ini?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
                        class="inline-flex items-center px-4 py-2 bg-red-600 dark:bg-red-600 border border-transparent rounded-lg font-semibold text-white hover:bg-red-700 dark:hover:bg-red-600 focus:bg-red-700 dark:focus:bg-red-600 active:bg-red-900 dark:active:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
