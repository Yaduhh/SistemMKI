...
<body>
    ...
    <div>
        ...
        <p>
            Tgl:
            {{ $pemasangan->tanggal_pemasangan ? \Carbon\Carbon::parse($pemasangan->tanggal_pemasangan)->format('d-M-y') : 'Belum ditentukan' }}
        </p>
    </div>
    ...
    <div>
        <p>Dengan Hormat,</p>
        <p style="margin-bottom: 10px; padding: 0;">Bersama ini kami sampaikan penawaran harga pemasangan sebagai
            berikut :</p>
    </div>
    @if (is_array($json_pemasangan) && count($json_pemasangan) > 0)
        ...
        <table>
            <thead>
                <tr>
                    <th colspan="6" class="title-produk" style="background: #577dbb; color: white;">Biaya Pemasangan
                    </th>
                </tr>
                <tr>
            ...
            <tbody>
                ...
                @foreach ($json_pemasangan as $section)
                    <tr>
                        <td colspan="6" class="title-produk" style="font-size:12px; font-weight: bold;">
                            {{ $section['sub_judul'] ?? '-' }}</td>
                    </tr>
                    @foreach ($section['items'] as $item)
                        @php
                            $qty = is_numeric($item['qty'])
                                ? $item['qty']
                                : floatval(str_replace([',', '.'], '.', preg_replace('/[^0-9.,]/', '', $item['qty'])));
                            $totalQty += $qty;
                        @endphp
                        <tr>
                            <td class="table-style" style="width: 5%; text-align: center;">{{ $no++ }}</td>
                            <td class="table-style" style="width: 35%;">{{ $item['item'] ?? '-' }}</td>
                            <td class="table-style" style="width: 10%; text-align: center;">
                                {{ $item['satuan'] ?? '-' }}</td>
                            <td class="table-style" style="width: 10%; text-align: center; font-weight: bold;">
                                {{ $item['qty'] ?? '-' }}</td>
        ...
        <table style="width: 100%;">
            <tr>
                <td class="table-style"
                    style="width: 5%; text-align: center; border: 0px 2px solid #000000 !important; border-top: 0px; padding-top: 10px; padding-bottom: 10px;">
                </td>
                <td class="table-style"
                    style="width: 35%; border: 0px 2px solid #000000 !important; border-top: 0px; padding-top: 10px; padding-bottom: 10px;">
                </td>
                <td class="table-style"
                    style="width: 10%; border: 0px 2px solid #000000 !important; border-top: 0px; padding-top: 10px; padding-bottom: 10px;">
                </td>
                <td class="table-style"
                    style="width: 10%; border: 0px 2px solid #000000 !important; border-top: 0px; padding-top: 5px; padding-bottom: 5px; text-align: center; font-weight: bold;">
                    {{ $totalQty }}</td>
                <td class="table-style"
                    style="width: 20%; border: 0px 2px solid #000000 !important; border-top: 0px; padding-top: 10px; padding-bottom: 10px;">
                </td>
                <td class="table-style"
                    style="border: 0px 2px solid #000000 !important; border-top: 0px; padding-top: 10px; padding-bottom: 10px;">
                </td>
            </tr>
            <tr>
                    ...
                    <table>
                        <tr>
                            ...
                            <td style="width: 50%; text-align: right; font-weight: bold;">
                                {{ number_format($pemasangan->total, 0, ',', '.') }}</td>
                        </tr>
                    </table>
                </td>
            </tr>
