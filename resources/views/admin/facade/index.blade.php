<x-layouts.app>
    ...
    <div class="py-6 space-y-6">
        <!-- Regular Facade Section -->
        <div class="w-full mx-auto">
            <x-card>
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Facade Regular</h3>
                </div>
                        ...
                        <tbody>
                            @forelse ($facades as $facade)
                                <tr class="bg-white border-b dark:bg-accent-foreground dark:border-gray-700">
                                    ...
                                    <td class="px-6 py-4">{{ $facade->nama_produk ?? '-' }}</td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">Rp {{ number_format($facade->harga ?? 0, 0, ',', '.') }}</td>
        ...
        <!-- Aksesoris Facade Section -->
        <div class="w-full mx-auto">
            <x-card>
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Facade Aksesoris</h3>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-accent-foreground dark:text-gray-400">
                        ...
                        <tbody>
                            @forelse ($aksesorisFacades as $facade)
                                <tr class="bg-white border-b dark:bg-accent-foreground dark:border-gray-700">
                                    <td class="px-6 py-4 text-center">{{ $loop->iteration }}</td>
                                    <td class="px-6 py-4">{{ $facade->code }}</td>
                                    <td class="px-6 py-4">{{ $facade->nama_produk ?? '-' }}</td>
