<x-layouts.app :title="__('Edit Rancangan Anggaran Biaya (RAB)')">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Add a small delay for validation initialization
            setTimeout(() => {
                validateMaterialUtama();
            }, 100);

            // Add event listeners for saturation and color inputs
        ...
        // Custom format for validation display (without Rp and currency syms if needed)
        ...
        function calculatePenawaranTotal() {
            let total = 0;
            const hiddenMaterialUtama = document.getElementById('json_pengeluaran_material_utama');
            if (!hiddenMaterialUtama) return 0;

            const isPintuPenawaran = hiddenMaterialUtama.value.includes('"judul_1"');
            
            if (isPintuPenawaran) {
                try {
                    const jsonData = JSON.parse(hiddenMaterialUtama.value);
        ...
        function calculateMaterialUtamaTotal() {
            ...
            if (isPintuPenawaran) {
                // For pintu penawaran, total is same as penawaran because it's non-editable
                return calculatePenawaranTotal();
            } else {
        ...
        function validateMaterialUtama() {
            ...
            document.getElementById('penawaran-total').textContent = `Rp ${formatNumber(penawaranTotal)}`;
            document.getElementById('rab-material-utama-total').textContent = `Rp ${formatNumber(rabTotal)}`;

            const validationBadge = document.getElementById('validation-badge');
            const validationText = document.getElementById('validation-text');

            if (Math.abs(penawaranTotal - rabTotal) < 0.01) {
                validationBadge.className = 'px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200';
                validationText.textContent = '✓ Tervalidasi';
            } else {
                validationBadge.className = 'px-3 py-1 rounded-full text-sm font-medium bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200';
        ...
        function prepareFormData() {
            ...
            const hiddenMaterialUtama = document.getElementById('json_pengeluaran_material_utama');
            if (hiddenMaterialUtama && !hiddenMaterialUtama.value.includes('"judul_1"')) {
                 hiddenMaterialUtama.value = JSON.stringify(materialUtamaData);
            }
        }
    </script>
</x-layouts.app>
